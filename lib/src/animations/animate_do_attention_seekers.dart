...
/// [fromX] from where you want to start the animation X
/// [fromY] from where you want to start the animation Y
/// [infinite] loops the animation until the widget is destroyed
class ShakeXY extends StatefulWidget {
  final Widget child;
  final Duration duration;
  final Duration delay;
  final bool infinite;
  ...
  @override
  ShakeXYState createState() => ShakeXYState();
}

/// State class,
/// Controls the animations flow
class ShakeXYState extends State<ShakeXY>
    with SingleTickerProviderStateMixin, AnimateDoState {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
        ...
        builder: (context, child) {
          return Transform.translate(
            offset: Offset(sin(4 * pi * shakeAnimation.value) * widget.fromX, sin(4 * pi * shakeAnimation.value) * widget.fromY),
            child: widget.child,
          );
        });
  }
...
/// [from] from where you want to start the animation
/// [infinite] loops the animation until the widget is destroyed
class ShakeY extends StatelessWidget {
  final Widget child;
  final Duration duration;
  final Duration delay;
  final bool infinite;
